...
a = xetrapal.karma.load_xpal_smriti(
    "/opt/karishmabot-appdata/karishmabotxpal.json")
a.save()
a.reload()
karishmatgbotxpal = xetrapal.Xetrapal(a)
karishmatgbotxpal.dhaarana(gdastras)
karishmatgbotxpal.dhaarana(smsastras)
#karishmatgbotgd = karishmatgbotxpal.gd_get_googledriver()
#sms = karishmatgbotxpal.get_sms_astra()
...
karishmatgbotxpal.logger.info("Setting up MongoEngine")
mongoengine.disconnect()
mongoengine.connect('karishmatgbot', alias='default')
...
def validate_member_dict(memberdict, new=True):
    ...
    if validation['status'] is True:
        karishmatgbotxpal.logger.info("memberdict: " + validation['message'])
    else:
        karishmatgbotxpal.logger.error("memberdict: " + validation['message'])
    return validation
...
def update_member_repayments(repayments):
    for r in repayments:
        ...
        karishmatgbotxpal.logger.info(mem.to_json())
        mem['repayments'][r['to']] += r['amount']
        mem.save()
        karishmatgbotxpal.logger.info(mem.to_json())
        mem2 = documents.Member.objects(username=r['to'])[0]
        karishmatgbotxpal.logger.info(mem2.to_json())
...
def get_xchange(xchange_id=None):
    karishmatgbotxpal.logger.info((xchange_id))
...
def create_xchange(xchangedict):
    karishmatgbotxpal.logger.info(utils.new_xchange_id())
    try:
